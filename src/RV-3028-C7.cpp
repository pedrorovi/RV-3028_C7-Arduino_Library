...
#define BUILD_MONTH                                                                                             \
    BUILD_MONTH_JAN | BUILD_MONTH_FEB | BUILD_MONTH_MAR | BUILD_MONTH_APR | BUILD_MONTH_MAY | BUILD_MONTH_JUN | \
        BUILD_MONTH_JUL | BUILD_MONTH_AUG | BUILD_MONTH_SEP | BUILD_MONTH_OCT | BUILD_MONTH_NOV | BUILD_MONTH_DEC
// <DATE>
...
char* RV3028::stringDateUSA() {
    ...
    sprintf(date, "%02hhu/%02hhu/20%02hhu", BCDtoDEC(_time[TIME_MONTH]), BCDtoDEC(_time[TIME_DATE]),
...
char* RV3028::stringDate() {
    ...
    sprintf(date, "%02hhu/%02hhu/20%02hhu", BCDtoDEC(_time[TIME_DATE]), BCDtoDEC(_time[TIME_MONTH]),
            BCDtoDEC(_time[TIME_YEAR]));
    return (date);
}
...
char* RV3028::stringTime() {
    ...
    if (is12Hour() == true) {
        ...
        sprintf(time, "%02hhu:%02hhu:%02hhu%cM", BCDtoDEC(_time[TIME_HOURS]), BCDtoDEC(_time[TIME_MINUTES]),
                BCDtoDEC(_time[TIME_SECONDS]), half);
    } else
        sprintf(time, "%02hhu:%02hhu:%02hhu", BCDtoDEC(_time[TIME_HOURS]), BCDtoDEC(_time[TIME_MINUTES]),
...
char* RV3028::stringTimeStamp() {
    ...
    if (is12Hour() == true) {
        ...
        sprintf(timeStamp, "20%02hhu-%02hhu-%02hhu  %02hhu:%02hhu:%02hhu%cM", BCDtoDEC(_time[TIME_YEAR]),
                BCDtoDEC(_time[TIME_MONTH]), BCDtoDEC(_time[TIME_DATE]), BCDtoDEC(_time[TIME_HOURS]),
                BCDtoDEC(_time[TIME_MINUTES]), BCDtoDEC(_time[TIME_SECONDS]), half);
    } else
        sprintf(timeStamp, "20%02hhu-%02hhu-%02hhu  %02hhu:%02hhu:%02hhu", BCDtoDEC(_time[TIME_YEAR]),
                BCDtoDEC(_time[TIME_MONTH]), BCDtoDEC(_time[TIME_DATE]), BCDtoDEC(_time[TIME_HOURS]),
...
If you want to set a weekday alarm (setWeekdayAlarm_not_Date = true), set 'date_or_weekday' from 0 (Sunday) to 6
(Saturday)
********************************/
